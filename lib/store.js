...
module.exports.Store = function(client) {
	...
	this.Category = function(name, category) {
		this.name = name;
		this.category = category;
	};
	Model.defineModel(this.Category, store, 'category', 'name', {
		name: Model.Type.Text,
		category: Model.Type.Text
	});
	this.Category.get = function*(commodity) {
		var rows = yield* store.querySync(
			' select category.category '
			+'from category where category.name = $1',
			[commodity]
		);
		if (!rows.length) return null;
	...
	this.create_tables = function*() {
		...
		yield* this.executeSync(' drop table if exists category ');
		...
		yield* this.executeSync(' create table category ( '
