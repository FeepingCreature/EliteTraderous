...
function elite_api_query(url, action, post_data, session) {
	...
	return new Promise(function(fulfill, reject) {
		...
		if (post_data) {
			...
			if (copy.hasOwnProperty('password')) copy['password'] = "<omitted>";
...
module.exports.Session = function() {
	...
	this.load_api_call = function*(url) {
		...
		if (this.need_stage('confirm')) {
			console.log("Attempt to quick-confirm login.");
			var thing = yield elite_api_query(EliteAPI.confirm_url, 'post', {'code': ''}, this);
